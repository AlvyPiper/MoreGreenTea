...
public:
	...
	virtual bool AreConVarsLinkable( const ConVar *child, const ConVar *parent ) = 0; // 8
};
...
public:
	...
	virtual void			RegisterConCommand( ConCommandBase *pCommandBase ) = 0; // 9
	virtual void			UnregisterConCommand( ConCommandBase *pCommandBase ) = 0; // 10
	virtual void			UnregisterConCommands( CVarDLLIdentifier_t id ) = 0; // 11

	// If there is a +<varname> <value> on the command line, this returns the value.
	// Otherwise, it returns NULL.
	virtual const char*		GetCommandLineValue( const char *pVariableName ) = 0; // 12
	virtual ConCommandBase *FindCommandBase( const char *name ) = 0; // 13 Try to find the cvar pointer by name
