...
public:
	...
	virtual void				ReadPixelsAsync(int x, int y, int width, int height, unsigned char* data, ImageFormat dstFormat, ITexture* texture, CThreadEvent* threadEvent) = 0;
	virtual int					ReadPixelsAsyncGetResult(int x, int y, int width, int height, unsigned char* data, ImageFormat dstFormat, CThreadEvent* threadEvent) = 0;
	// Sets lighting
	...
	virtual void				MultMatrixLocal(matrix3x4_t const& matrix) = 0; // 30
	...
	virtual void OverrideDepthEnable(bool bEnable, bool bDepthEnable) = 0; // 80
	...
	virtual void SetFlashlightState(const FlashlightState_t &state, const VMatrix &worldToTexture) = 0; // 90
