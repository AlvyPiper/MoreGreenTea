...
{
public:
	PFHandle() : m_iPanelID(INVALID_PANEL) {} //m_iSerialNumber(0), m_pListEntry(0) {}

	FPanel *Get();
	FPanel *Set(FPanel *pPanel);
	FPanel *Set(HPanel hPanel);

	operator FPanel *()						{ return Get(); }
	FPanel * operator ->()					{ return Get(); }
	FPanel * operator = (FPanel *pPanel)		{ return Set(pPanel); }

	bool operator == (FPanel *pPanel)		{ return (Get() == pPanel); }
	operator bool()						{ return Get() != 0; }

	friend inline bool operator == (const PFHandle &p1, const PFHandle &p2)
	{
		return p1.m_iPanelID == p2.m_iPanelID;
	}
