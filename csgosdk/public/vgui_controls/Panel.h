...
public:
	...
	virtual void InstallMouseHandler(vgui::Panel* panel);
	// drawing state
	...
	virtual void SetBorder(IBorder *border); // border
	...
	// interface to build settings takes a group of settings and applies them to the control
	...
	int				GetKeyMappingCount( ); // Returns the number of keybindings
	void			RevertKeyBindingsToDefault();
	void			RemoveAllKeyBindings();
	void			ReloadKeyBindings(); 
	void			SaveKeyBindingsToBuffer(int level, CUtlBuffer& buf); // calls RevertKeyBindingsToDefault() and then LoadKeyBindingsForOnePanel( GetKeyBindingsContext(), this );
	bool			ParseKeyBindings(KeyValues *kv);
	void			SetAllowKeyBindingChainToParent(bool state); // Set this to false to disallow IsKeyRebound chaining to GetParent() Panels...
	...
	// allows you to disable for this panel but not children //virtual bool HandleMouseCode( MouseCode code );
	...
	virtual void DrawHollowBox( int x, int y, int wide, int tall, Color color, float normalizedAlpha, int, int );
	...
	void		SetMessageContextId_R( int nContextID ); // Forces context ID for this panel and all children below it
...
public:
	...
	*/
	...
	enum PinCorner_e  {
		PIN_TOPLEFT = 0, PIN_TOPRIGHT, PIN_BOTTOMLEFT, PIN_BOTTOMRIGHT, PIN_NO,
		PIN_CENTER_TOP, PIN_CENTER_RIGHT, PIN_CENTER_BOTTOM, PIN_CENTER_LEFT,
	};
	// specifies the auto-resize directions for the panel
	enum AutoResize_e { AUTORESIZE_NO = 0, AUTORESIZE_RIGHT, AUTORESIZE_DOWN, AUTORESIZE_DOWNANDRIGHT, };
