...
public:
	...
	//virtual void				otherdeconstructor( void ); // 1 not defined here
	virtual	bool				IsCommand( void ) const; // 2
	virtual bool				IsFlagSet( int flag ) const; // Check flag 3
	virtual void				AddFlags( int flags ); // Set flag 4
	virtual void				RemoveFlags( int flags ); // Clear flag 5
	virtual int					GetFlags() const; // 6
	virtual const char			*GetName( void ) const; // Return name of cvar 7
	virtual const char			*GetHelpText( void ) const; // Return help text for cvar 8
	virtual bool				IsRegistered( void ) const; // 9
	virtual CVarDLLIdentifier_t	GetDLLIdentifier() const; // 10
...
public: // non virtual
	...
	void						Shutdown();
	char						*CopyString( const char *from ); // Internal copy routine
...
public:
	...
	ConCommandBase				*m_pNext; // 0x4 cause virtual table
