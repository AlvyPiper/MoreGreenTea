...
typedef struct player_info_s
{
	...
	char __pad1[0x10];
	char name[MAX_PLAYER_NAME_LENGTH];//80
	char __pad2[0x64];
	char guid[SIGNED_GUID_LEN + 1];
	char __pad3[0x17B];
...
public:
	...
	// Encode the delta (changes) between the CUserCmd in slot from vs the one in slot to.  The game code will have matching logic to read the delta.
