...
public:
	...
	virtual bool IsAllowedToSwitch(void) { return true; }
	...
	virtual void SetWeaponIdleTime(float time) { m_flTimeWeaponIdle = time; }
	virtual float GetWeaponIdleTime(void) { return m_flTimeWeaponIdle; }

	// Weapon selection
	virtual bool HasAnyAmmo() // Returns true if weapon has ammo
	{
		// If I don't use ammo of any kind, I can always fire
		if (!UsesPrimaryAmmo() && !UsesSecondaryAmmo()) return true;
	...
	virtual void SetActivity(Activity act, float duration)
	{
		...
		if (sequence == ACTIVITY_NOT_AVAILABLE) // FORCE IDLE on sequences we don't have (which should be many)
...
inline float CBaseCombatWeapon::GetInaccuracy()
{
	//typedef float(__thiscall* GetInaccuracyFn)(void*);
	//GetInaccuracyFn Inaccuracy = (GetInaccuracyFn)((*(PDWORD_PTR*)this)[459]);
	//return Inaccuracy(this);
}

inline float CBaseCombatWeapon::GetSpread()
{
	//typedef float(__thiscall* GetSpreadFn)(void*);
	//GetSpreadFn Spread = (GetSpreadFn)((*(PDWORD_PTR*)this)[460]);
	//return Spread(this);
}

inline void CBaseCombatWeapon::UpdateAccuracyPenalty()
{
	//typedef void(__thiscall* UpdateAccuracyPenaltyFn)(void*);
	//UpdateAccuracyPenaltyFn Update = (UpdateAccuracyPenaltyFn)((*(PDWORD_PTR*)this)[461]);
	//return Update(this);
}

inline std::string CBaseCombatWeapon::WeaponName()
{
	std::string wpnClassname(this->GetClientClass()->GetName());
	if (wpnClassname.find("Weapon") != std::string::npos)
		return wpnClassname.substr(7, wpnClassname.length() - 7); //trim CWeapon
	else
		return wpnClassname.substr(1, wpnClassname.length() - 1); //trim C
	return wpnClassname;
}
