...
public:
	...
	virtual void	OverrideAudioState(AudioState_t *pAudioState) = 0; // 19
	virtual int		KeyInput(int down, ButtonCode_t keynum, const char *pszCurrentBinding) = 0; // 20
	virtual void	StartMessageMode(int iMessageModeType) = 0;
	virtual vgui::Panel *GetMessagePanel() = 0;
	virtual void	OverrideMouseInput(float *x, float *y) = 0;
	virtual bool	CreateMove(float flInputSampleTime, CUserCmd *cmd) = 0; // 24
	...
	virtual void	PreRender(CViewSetup *pSetup) = 0; // 30
	...
	virtual void		SetMapName(char* name) = 0; // 41
	...
	virtual void		DoPostScreenSpaceEffects(const CViewSetup *pSetup) = 0; // 45
	// Updates.
public:
	// Called every frame.
	...
	virtual void	FireGameEvent(IGameEvent* gameEvent) = 0;
};
